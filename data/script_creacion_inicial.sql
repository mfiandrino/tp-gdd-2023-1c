...
CREATE TABLE BASE_DE_GATOS_2.ENVIO_MENSAJERIA (
	...
	-- FKs
	EM_DIR_ORIGEN decimal(18, 0) not null,
	EM_DIR_DEST decimal(18, 0) not null,
...
ADD CONSTRAINT FK_DIR_ORIGEN_ENVIO_MENSAJERIA FOREIGN KEY (EM_DIR_ORIGEN) REFERENCES BASE_DE_GATOS_2.DIRECCION (DIRECCION_ID),
CONSTRAINT FK_DIR_DESTINO_ENVIO_MENSAJERIA FOREIGN KEY (EM_DIR_DEST) REFERENCES BASE_DE_GATOS_2.DIRECCION (DIRECCION_ID),
CONSTRAINT FK_ESTADO_ENVIO_MENSAJERIA FOREIGN KEY (EM_ESTADO) REFERENCES BASE_DE_GATOS_2.ESTADO_ENVIO_MENSAJERIA (ESTADO_EM_ID),
...
CREATE PROCEDURE BASE_DE_GATOS_2.migrar_envio_mensajeria
	AS
		...
		INSERT INTO BASE_DE_GATOS_2.ENVIO_MENSAJERIA(
			EM_NUMERO, EM_DIR_ORIGEN, EM_DIR_DEST, EM_KILOMETROS, EM_VALOR_ASEGURADO, EM_PRECIO_ENVIO, EM_PRECIO_SEGURO,
		...
		EM_REPARTIDOR, EM_MEDIO_DE_PAGO)
			SELECT DISTINCT
				...
				d.DIRECCION_ID DIR_ORIG,
				d2.DIRECCION_ID DIR_DEST,
			...
			JOIN BASE_DE_GATOS_2.LOCALIDADES l 
				ON m.ENVIO_MENSAJERIA_LOCALIDAD = l.LOCALIDAD_NOMBRE AND l.LOCALIDAD_PROVINCIA_ID = p.PROVINCIA_ID
			JOIN BASE_DE_GATOS_2.DIRECCION d 
				ON m.ENVIO_MENSAJERIA_DIR_ORIG = d.DIRECCION_DIRECCION AND d.LOCALIDAD_ID = l.LOCALIDAD_ID
			JOIN BASE_DE_GATOS_2.DIRECCION d2 
				ON m.ENVIO_MENSAJERIA_DIR_DEST = d2.DIRECCION_DIRECCION AND d2.LOCALIDAD_ID = l.LOCALIDAD_ID
			...
			JOIN BASE_DE_GATOS_2.MEDIOS_DE_PAGO mp
				ON u.USUARIO_ID = mp.MP_USUARIO AND
				tmp.TIPO_MEDIO_PAGO_ID = mp.MP_TIPO AND
	...
	AS
		...
		INSERT INTO BASE_DE_GATOS_2.RECLAMO_CUPON(CUPON_NUMERO, RECLAMO_NUMERO, RECLAMO_CUPON_TIPO_ID)
			SELECT DISTINCT
				...
				m.RECLAMO_NRO,
				(
					SELECT TOP 1 t.RECLAMO_CUPON_TIPO_ID
					FROM BASE_DE_GATOS_2.RECLAMO_CUPON_TIPO t
					WHERE m.CUPON_RECLAMO_TIPO = t.RECLAMO_CUPON_TIPO
				)
			FROM 
				gd_esquema.Maestra m
			WHERE
